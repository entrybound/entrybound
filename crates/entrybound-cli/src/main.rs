...
\n\
Usage:\n\
  entrybound pack <input-directory> [output.eb]\n\
  entrybound unpack <archive.eb> [destination]\n\
  entrybound list <archive.eb>\n\
  entrybound inspect <archive.eb>\n\
  entrybound verify <archive.eb>\n\
\n\
...
mod tests {
    ...
    fn help_is_available() {
        assert!(run(args(&["entrybound"])).is_ok());
    }

    #[test]
    fn future_commands_fail_explicitly() {
        let error = run(args(&["entrybound", "convert"])).unwrap_err();
