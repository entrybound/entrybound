...
ratio. It also bounds observations globally/per subject, conflicts, resolutions,
conversion/preservation bytes, and exact preserved-source bytes. Every
offset/length operation is checked before slicing or allocation.
...
ebound convert input.zip output-python.eb --compat=zip/python-zipfile@3.13.5
ebound convert input.zip output-preserved.eb --preserve \
...
Strict is the default. Detection uses valid ZIP end/record structure rather
than the filename extension; `--from zip` is an explicit assertion. Compatibility
IDs are exact-version contracts. Preserve requires compat and stores exact
source/LOM evidence. `--dry-run` performs observation, reconciliation, content
decoding/verification, and native planning but creates no output. Output is a
normal unencrypted native `.eb`, so ordinary verify/list/inspect/unpack apply.
Encrypted conversion, ZIP export, encrypted ZIP, and symlink/special-file EAM
...
results are checked in under `tools/zip-compat` and cannot silently change this
policy or a frozen compatibility profile.

The development corpus records the following qualitative differences (tool
the checked JSON records exact tool versions):
...
The complete machine-readable matrix and generated constructors live in
`tools/zip-compat/`; production code encodes reviewed rules and never invokes a
reference runtime. See
[zip-compatibility-profiles-v1.md](zip-compatibility-profiles-v1.md).
