...
| B1. Infrastructure & corpus | §4, §5, §6, §7 | PARTIAL — `wf_3615a3d7-fe4` (`phase-b1d-corpus-baselines.js`) completed; corpus closure RUNNING as `wf_a924ee34-6e0` (`phase-b1e-corpus-closure.js`: g2 blockers and g4 majors, ebr backslash fix, round-2 assembly, round-2 critic; Sonnet); the detached size pass continues | DONE: environment, `ebr` runner, and corpus framework; all 5 corpus groups provisioned; round-0 assembly `6400fc3` (196 items, manifest `a71e7ee1…`); corpus critic `research/corpus/critique-round1.md` (INCOMPLETE: 25 gaps, 7 BLOCKER); gap fixes committed by g1–g5 (e.g. ML weights/parquet, DB dumps, real git clones, supply-chain signatures/SBOMs, real duplicate/sparse/many-small-file trees, F12 held-out independence). Baselines `f516306`: 27 incumbent configs, `capability-matrix.csv` (298 rows), probes. **OPEN:** round-1 assembly not regenerated (manifest stale). Some declared items are unmaterialized or stale (`f05-validation-gen-journal-systemd` logical hash). Blockers remain: F19 real home backups, F19 real cross-platform metadata, F20 encrypted private archives. F14/F16/F09/F10/F11 major gaps closed 2026-09-17 (see change log); the F20 structural-adversarial generators are WIP. The EXP-BASE-SIZE size pass runs detached (`research/baselines/run_size_pass.sh`). |
