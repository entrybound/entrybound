//! FastCDC 2020: the same Gear-hash normalized-chunking boundary rule as
//! [`super::fastcdc2016`], restructured to consume two bytes per iteration
//! ("rolling two bytes each time") for speed, while producing the same cut
//! points.
//!
//! Xia, W., Zhou, Y., Jiang, H., Feng, D., Hua, Y., Hu, Y., Zhang, Y., & Liu,
//! Q. (2020). *FastCDC 2.0: A Simple, Efficient, and Scalable Content-Defined
//! Chunking Approach.* IEEE Transactions on Parallel and Distributed Systems.
//!
//! As with [`super::fastcdc2016`], this ports the algorithm's `cut_gear`
//! decision rather than re-deriving it, and reuses the exact `GEAR`,
//! `GEAR_LS` (`GEAR` left-shifted by one bit, section 3.7 of the 2020 paper)
//! and `MASKS` tables from the `fastcdc` crate (pinned as a dev-dependency;
//! see [`super::fastcdc2016`]'s module docs for the tables' provenance).
//! `tests` below cross-check this port's boundaries against that crate's
//! `v2020::FastCDC`, and separately assert that this module and
//! [`super::fastcdc2016`] produce byte-identical boundaries for the same
//! parameters -- exactly the paper's own claim about what changed (speed,
//! not cut points).
//!
//! This module implements only the unseeded variant (the paper's
//! `with_level_and_seed`'s seed left at zero); the seeded Gear-table variant
//! exists in the 2020 paper (and the reference crate) to make chunking
//! boundaries harder to infer from outside without the seed, which is not a
//! property this research harness's comparisons need.
//!
//! Not wired into any production code path (see [`super::fastcdc2016`]'s
//! module docs for how production `gear-norm-v1` differs).

use super::{ChunkRange, round_log2};
use entrybound::chunker::ChunkingParameters;

pub(crate) use super::fastcdc2016::MASKS;

//
// GEAR: identical table to `super::fastcdc2016::GEAR` (see its module docs
// for provenance); duplicated here (rather than shared) so this module is a
// direct, independently-readable port of the `fastcdc` crate's `v2020`
// module, matching that crate's own layout (which also duplicates the
// table between `v2016` and `v2020`).
//
#[rustfmt::skip]
const GEAR: [u64; 256] = [
...
//
// GEAR_LS: GEAR left-shifted by 1 bit (section 3.7 of the FastCDC 2020
// paper): `GEAR_LS[b] == GEAR[b] << 1`. Verbatim from the `fastcdc` crate's
// `v2020::GEAR_LS` (see module docs for provenance).
//
#[rustfmt::skip]
...
pub use super::fastcdc2016::Normalization;
...
impl Params {
    ...
    fn validate(&self) -> Result<(), String> {
        if self.min_size == 0 || self.min_size > self.avg_size || self.avg_size > self.max_size {
            return Err(format!(
                "invalid fastcdc-2020 parameters: min {} avg {} max {}",
        ...
        if bits < n || (bits + n) as usize >= MASKS.len() {
            return Err(format!(
                "fastcdc-2020 avg_size {} with normalization {} is out of the published mask table's range",
                self.avg_size, n
            ));
        }
        Ok(())
...
/// Ports `fastcdc` crate's `v2020::cut_gear` verbatim (unseeded GEAR/GEAR_LS
/// tables only -- see module docs).
...
/// Finds FastCDC-2020 ranges. Empty input has no ranges. Panics if `params`
/// fails validation (build via [`Params::from_profile`]).
#[must_use]
...
mod tests {
    ...
    fn matches_the_pinned_fastcdc_crate_for_every_production_profile() {
        ...
        let data = pseudo_random_bytes(6_000_000, 43);
        for profile in [FAST_V2, BALANCED_V2, DENSE_V2, EXTREME_V2] {
            ...
            let reference: Vec<(usize, usize)> = fastcdc::v2020::FastCDC::with_level(
                &data,
                params.min_size,
                params.avg_size,
                params.max_size,
            ...
            .collect();
            let ours_pairs: Vec<(usize, usize)> = ours.iter().map(|r| (r.start, r.end)).collect();
            assert_eq!(
                ours_pairs, reference,
                "boundaries differ from the pinned fastcdc crate for {:?}",
                profile.chunker_id
            );
        }
    }

    /// The 2020 paper's own claim: restructuring to two bytes per iteration
    /// changes throughput, not cut points. Same parameters, same data, same
    /// boundaries as `fastcdc2016`.
    #[test]
    fn produces_the_same_boundaries_as_fastcdc_2016() {
        use crate::algorithms::fastcdc2016;
        let data = pseudo_random_bytes(500_000, 21);
        let params_2020 = test_params(1024, 8192, 65536);
        let params_2016 = fastcdc2016::Params {
            min_size: params_2020.min_size,
            avg_size: params_2020.avg_size,
            max_size: params_2020.max_size,
            normalization: fastcdc2016::Normalization::Level1,
        };
        let a = chunk_ranges(&data, &params_2020);
        let b = fastcdc2016::chunk_ranges(&data, &params_2016);
        assert_eq!(a, b);
    }

    #[test]
    fn insertion_only_perturbs_nearby_boundaries() {
        let mut data = pseudo_random_bytes(400_000, 8);
        let insert_at = 5_000;
        let insertion = pseudo_random_bytes(37, 100);
        let params = test_params(1024, 8192, 65536);
