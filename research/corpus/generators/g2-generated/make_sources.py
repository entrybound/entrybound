...
# Corpus round-1 critic gap (F19 real home/project-directory backups, BLOCKER): F19 had no real
# mixed personal/office document tree, and no working copy anywhere in the corpus keeps its .git
# (every F01 item is `git archive` or a tarball). Each item below combines, in one recipe: a
# genuine full `git clone` (recipe.git, history=full -- real refs/packed-objects/checked-out tree,
# not an archive) of a small, real, permissively-licensed public repository; a real, published
# personal "dotfiles" repository's tarball (recipe.inputs, TOFU-pinned, GitHub has no published
# hash for archive tarballs); and a real, already-provisioned, public-domain government PDF item
# from family F13 in the SAME split (recipe.from_items + copy-item, no new bytes fetched). The
# run step (home_snapshot.py) only reorganizes this real content into a realistic home-directory
# layout (Projects/<repo>/ with .git intact, dotfiles spread across the home root, Documents/ with
# the real PDF) plus a modest amount of clearly-synthetic filler (Pictures/Music/Downloads
# duplicate/.cache/.local/share/Trash/a browser-profile SQLite) for realistic breadth, then writes
# TWO dated backup snapshots with real edits/a deletion/an addition between them, every unchanged
# file hardlinked between snapshots exactly as a real rsync --link-dest/rsnapshot run would leave
# them (verified below by inode comparison, not merely asserted).
ITEMS += [
    item("f19-tuning-real-home-snapshot", "F19", "tuning", "medium", "git-archive", "derived-from-real",
         {"git": {"repo": "https://github.com/isaacs/node-mkdirp.git",
                 "commit": "fdf6005432c2506ce31f8cae237039dda2f8479d", "ref": "main", "history": "full"},
          "output_pin": None,
          "inputs": [{"name": "dotfiles-src",
                     "url": "https://github.com/mathiasbynens/dotfiles/archive/"
                            "b7c7894e7bb2de5d60bfb9a2f5e46d01a61300ea.tar.gz",
                     "sha256": "TOFU", "notes": "GitHub archive tarball; no upstream-published hash, hence TOFU"}],
          "from_items": ["f13-tuning-nasa-ntrs-pdf-small"],
          "steps": [{"op": "extract", "input": "dotfiles-src", "format": "tar", "dest": "raw-inputs/dotfiles",
                    "strip_components": 1},
                    {"op": "copy-item", "from_item": "f13-tuning-nasa-ntrs-pdf-small", "dest": "raw-inputs/documents"},
                    {"op": "run", "script": f"{GEN}/home_snapshot.py", "interpreter": "python", "seed": 73001,
                     "params": {"variant": "tuning", "project_repo": "node-mkdirp", "project_license": "MIT",
                               "dotfiles_repo": "mathiasbynens/dotfiles", "dotfiles_license": "MIT",
                               "document_source_item": "f13-tuning-nasa-ntrs-pdf-small"}}]},
         lic("MIT (project + dotfiles) AND LicenseRef-US-Government-Work (documents)", True,
             "James Halliday and Isaac Z. Schlueter (node-mkdirp); Mathias Bynens (dotfiles); NASA STI Program "
             "(NTRS document, reused from f13-tuning-nasa-ntrs-pdf-small)",
             "Composite item: see MANIFEST.json/PROVENANCE.txt inside the item for the exact per-component "
             "breakdown. All three real components are independently redistributable (MIT; US Government Work). "
             "Pictures/Music/.cache/browser-profile/Trash filler is synthetic, CC0-equivalent, generated by this "
             "item's own run step."),
         "nodejs-mkdirp-home-snapshot",
         "Real home/project-directory backup series: a genuine full `git clone` of isaacs/node-mkdirp (real "
         ".git, checked-out working tree) under Projects/, a real published dotfiles repository "
         "(mathiasbynens/dotfiles) spread across the home root as actual dotfiles, and a real NASA NTRS PDF "
         "(reused from F13) under Documents/ -- reorganized into two dated backup snapshots "
         "(backup-2026-02-15, backup-2026-03-01) with real edits/a deletion/an addition between them and "
         "every unchanged file hardlinked across snapshots.",
         tags=["derived-from-real", "home-snapshot", "git-working-copy"]),
    item("f19-validation-real-home-snapshot", "F19", "validation", "small", "git-archive", "derived-from-real",
         {"git": {"repo": "https://github.com/chalk/ansi-styles.git",
                 "commit": "c1c3dd4e017a2938807aaff0d361f46d086aeab7", "ref": "main", "history": "full"},
          "output_pin": None,
          "inputs": [{"name": "dotfiles-src",
                     "url": "https://github.com/holman/dotfiles/archive/"
                            "9a90082b61fa5659789b9d41544c19098825a3ca.tar.gz",
                     "sha256": "TOFU", "notes": "GitHub archive tarball; no upstream-published hash, hence TOFU"}],
          "from_items": ["f13-validation-federal-register-pdf-small"],
          "steps": [{"op": "extract", "input": "dotfiles-src", "format": "tar", "dest": "raw-inputs/dotfiles",
                    "strip_components": 1},
                    {"op": "copy-item", "from_item": "f13-validation-federal-register-pdf-small",
                     "dest": "raw-inputs/documents"},
                    {"op": "run", "script": f"{GEN}/home_snapshot.py", "interpreter": "python", "seed": 73002,
                     "params": {"variant": "validation", "project_repo": "ansi-styles", "project_license": "MIT",
                               "dotfiles_repo": "holman/dotfiles", "dotfiles_license": "MIT",
                               "document_source_item": "f13-validation-federal-register-pdf-small"}}]},
         lic("MIT (project + dotfiles) AND LicenseRef-US-Government-Work (documents)", True,
             "Sindre Sorhus (chalk/ansi-styles); Zach Holman (dotfiles); the Office of the Federal Register "
             "(document, reused from f13-validation-federal-register-pdf-small)",
             "Composite item: see MANIFEST.json/PROVENANCE.txt inside the item for the exact per-component "
             "breakdown. All three real components are independently redistributable (MIT; US Government Work). "
             "Synthetic filler as in the tuning item."),
         "chalk-ansistyles-home-snapshot",
         "Real home/project-directory backup series (independent upstreams from tuning): a genuine full `git "
         "clone` of chalk/ansi-styles under Projects/, holman/dotfiles spread across the home root, and a real "
         "Federal Register PDF (reused from F13) under Documents/ -- two dated backup snapshots with real "
         "edits/a deletion/an addition and hardlinked-unchanged files between them.",
         tags=["derived-from-real", "home-snapshot", "git-working-copy"]),
    item("f19-heldout-real-home-snapshot", "F19", "heldout", "small", "git-archive", "derived-from-real",
         {"git": {"repo": "https://github.com/jonschlinkert/is-number.git",
                 "commit": "98e8ff1da1a89f93d1397a24d7413ed15421c139", "ref": "master", "history": "full"},
          "output_pin": None,
          "inputs": [{"name": "dotfiles-src",
                     "url": "https://github.com/necolas/dotfiles/archive/"
                            "60563f43fdb1db9c5e498a8745a75900574b69b7.tar.gz",
                     "sha256": "TOFU", "notes": "GitHub archive tarball; no upstream-published hash, hence TOFU"}],
          "from_items": ["f13-heldout-usgs-pdf-small"],
          "steps": [{"op": "extract", "input": "dotfiles-src", "format": "tar", "dest": "raw-inputs/dotfiles",
                    "strip_components": 1},
                    {"op": "copy-item", "from_item": "f13-heldout-usgs-pdf-small", "dest": "raw-inputs/documents"},
                    {"op": "run", "script": f"{GEN}/home_snapshot.py", "interpreter": "python", "seed": 73003,
                     "params": {"variant": "heldout", "project_repo": "is-number", "project_license": "MIT",
                               "dotfiles_repo": "necolas/dotfiles", "dotfiles_license": "MIT",
                               "document_source_item": "f13-heldout-usgs-pdf-small"}}]},
         lic("MIT (project + dotfiles) AND LicenseRef-US-Government-Work (documents)", True,
             "Jon Schlinkert (is-number); Nicolas Gallagher (dotfiles); the U.S. Geological Survey (document, "
             "reused from f13-heldout-usgs-pdf-small)",
             "Composite item: see MANIFEST.json/PROVENANCE.txt inside the item for the exact per-component "
             "breakdown. All three real components are independently redistributable (MIT; US Government Work). "
             "Synthetic filler as in the tuning item."),
         "jonschlinkert-isnumber-home-snapshot",
         "Held-out: a real home/project-directory backup series with independent upstreams from every "
         "tuning/validation item -- a genuine full `git clone` of jonschlinkert/is-number under Projects/, "
         "necolas/dotfiles spread across the home root, and a real USGS PDF (reused from F13, same split) "
         "under Documents/ -- two dated backup snapshots with real edits/a deletion/an addition and "
         "hardlinked-unchanged files between them.",
         notes="Held-out: distinct independence group and distinct upstream project/dotfiles-repo/document "
               "from every tuning/validation F19 item.",
         tags=["derived-from-real", "home-snapshot", "git-working-copy"]),
]
