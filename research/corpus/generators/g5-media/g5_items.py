...
LIC = {
    ...
    "nasa-video": {"spdx_or_name": "LicenseRef-NASA-media-guidelines (US Government work, public domain in the US)", "redistributable": True,
                   "attribution": "NASA; center credit per input notes",
                   "notes": "Round-1 fix G21 (F13 non-benchmark large held-out item + new production pipeline): a NASA Image and "
                            "Video Library video, same public-domain basis as nasa-ivl still photographs (F12); selection excluded "
                            "records whose credits name commercial or non-NASA-agency parties (see select_sources.py pool_nasa_video_naca)."},
}
...
def build_items(sel, partial=False):
    ...
    A(lambda: download_item("f13-tuning-bbb-video-medium", "F13", "tuning", "medium", "g5-blender-bbb",
                    ...
                    tags=["public-benchmark", "video", "pipeline:blender-open-movie"], extract_zip=True))
    ...
    A(lambda: download_item("f13-tuning-prelinger-video-medium", "F13", "tuning", "medium", "g5-prelinger-archives",
                    ...
                    "Theora/Vorbis derivatives, stored as <identifier>/<file>.", tags=["video", "pipeline:prelinger-archive-org"],
    ...
    A(lambda: download_item("f13-tuning-musopen-flac-medium", "F13", "tuning", "medium", "g5-musopen-kickstarter-flac",
                    ...
                    "the Musopen Kickstarter lossless compilation.", tags=["audio", "lossless-audio", "pipeline:musopen"]))
    A(lambda: download_item("f13-tuning-nasa-ntrs-pdf-small", "F13", "tuning", "small", "g5-nasa-ntrs-pdf",
                    ...
                    tags=["pdf", "pipeline:nasa-ntrs-pdf"]))
    A(lambda: download_item("f13-tuning-commons-media-small", "F13", "tuning", "small", "g5-commons-media-set-a",
                    ...
                    tags=["mixed-media", "pipeline:commons-media"]))
    A(lambda: build_item("f13-tuning-kodak-image-codecs-small", "F13", "tuning", "small", "g5-kodak-photocd-suite",
                 ...
                 tags=["public-benchmark", "image-codecs", "pipeline:image-codecs-matrix"]))
    ...
    A(lambda: download_item("f13-validation-sintel-media-medium", "F13", "validation", "medium", "g5-blender-sintel",
                    ...
                    "480p/720p, plus the film's 5.1 music+effects stem as FLAC.",
                    tags=["public-benchmark", "video", "lossless-audio", "pipeline:blender-open-movie"]))
    # Round-1 fix G21 (F13 validation large tier): "higher Sintel renditions" than the trailer --
    # the complete film (durian/movies/, not durian/trailer/) at up to 1080p, same independence
    # group as the item above (same film/pipeline, just the full-length release).
    A(lambda: download_item("f13-validation-sintel-media-large", "F13", "validation", "large", "g5-blender-sintel",
                    entries_for(sel, "sintel-full-movie", "f13-validation-sintel-media-large"), LIC["sintel"],
                    "Sintel (Blender open movie) full-length renditions from download.blender.org/durian/movies/ (not the trailer "
                    "directory used above): the 720p MKV (stored ZIP extracted) and the 1080p MKV, both full-length H.264/AC3 "
                    "releases at far higher bitrate than the ~1-minute trailer.",
                    tags=["public-benchmark", "video", "pipeline:blender-open-movie"], extract_zip=True))
    A(lambda: download_item("f13-validation-librivox-audiobook-medium", "F13", "validation", "medium", "g5-librivox-tenn-sf-stories",
                    ...
                    "3sfstoriesbywilliamtenn_1910_librivox.", tags=["audio", "pipeline:librivox"]))
    A(lambda: download_item("f13-validation-federal-register-pdf-small", "F13", "validation", "small", "g5-federal-register-pdf",
                    ...
                    "dates, as published on govinfo.gov; <date>/<type>/<document number>.pdf.", tags=["pdf", "pipeline:federal-register-pdf"]))
    A(lambda: download_item("f13-validation-commons-media-small", "F13", "validation", "small", "g5-commons-media-set-b",
                    ...
                    "WAV, MP3, WebM.", tags=["mixed-media", "pipeline:commons-media"]))
    A(lambda: build_item("f13-validation-fsa-image-codecs-small", "F13", "validation", "small", "g5-loc-fsa-owi-color", sorted(fsa, key=lambda e: e["key"])[:2],
                 ...
                 tags=["image-codecs", "pipeline:image-codecs-matrix"]))
    # Round-1 fix G21 (F13 distinct-pipeline media): phone-style HEIC stills + a short synthetic
    # HEVC pan/zoom clip (x265/libheif matrix) over the same-split FSA/OWI TIFFs -- a production
    # pipeline (modern smartphone camera output) that no other F13 item uses. Generated because the
    # "video" motion is a seeded synthetic pan, not a real recording.
    A(lambda: build_item("f13-validation-phone-matrix-small", "F13", "validation", "small", "g5-loc-fsa-owi-color",
                 sorted(fsa, key=lambda e: e["key"])[:2], LIC["fsa"],
                 "Phone-style HEIC stills (heif-enc/libheif + x265 intra, two quality levels, one with an embedded thumbnail) and a "
                 "short synthetic landscape+portrait HEVC clip (libx265, Main profile, hvc1 tag, seeded ffmpeg zoompan pan/zoom) per "
                 "image, box-downscaled to a phone-camera-scale resolution first; heic/<image>-q<N>.heic, video/<image>-<orientation>-"
                 "<w>x<h>.mp4.",
                 "phone_matrix.py", 2401,
                 {"inputs": lossless_names(sorted(fsa, key=lambda e: e["key"])[:2]), "long_edge": 4032, "heic_qualities": [35, 55],
                  "heic_thumb_at": 1, "video": {"seconds": 2, "fps": 30, "crf": 30, "keyint": 60, "resolutions": [[1920, 1080], [1080, 1920]]},
                  "require_tools": {"ffmpeg": "ffmpeg version 6.1.1-3ubuntu5", "heif-enc": "1.17.6", "pillow": "Pillow 12.3.0"},
                  "max_bytes": 15 * MiB},
                 tags=["phone-style-matrix", "image-codecs", "video", "generated", "pipeline:phone-x265-heif"],
    ...
    A(lambda: download_item("f13-heldout-tos-media-large", "F13", "heldout", "large", "g5-blender-tears-of-steel",
                    ...
                    "stereo mix AIFF. Provenance-only description.",
                    tags=["public-benchmark", "video", "pipeline:blender-open-movie"], extract_zip=True))
    # Elephants Dream is, like Tears of Steel above, a well-known Blender Foundation open movie
    # (round-1 corpus critique: it "comes from the same Blender open-movie set but is not tagged"
    # public-benchmark) -- retagged here for consistency, and given the same pipeline covariate.
    A(lambda: download_item("f13-heldout-elephants-dream-media-medium", "F13", "heldout", "medium", "g5-blender-elephants-dream",
                    ...
                    "the 5.1 music+effects AC-3 stream. Provenance-only description.",
                    tags=["public-benchmark", "video", "pipeline:blender-open-movie"]))
    A(lambda: download_item("f13-heldout-librivox-audiobook-medium", "F13", "heldout", "medium", "g5-librivox-reynolds-sf-stories",
                    ...
                    "Provenance-only description.", tags=["audio", "pipeline:librivox"]))
    A(lambda: download_item("f13-heldout-musopen-chopin-flac-medium", "F13", "heldout", "medium", "g5-musopen-chopin-flac",
                    ...
                    tags=["audio", "lossless-audio", "pipeline:musopen"]))
    A(lambda: download_item("f13-heldout-usgs-pdf-small", "F13", "heldout", "small", "g5-usgs-pubs-pdf",
                    ...
                    "PDF reports from the USGS Publications Warehouse numbered series (2020-2023). Provenance-only description.",
                    tags=["pdf", "pipeline:usgs-pdf"]))
    ...
    A(lambda: build_item("f13-heldout-highsmith-image-codecs-small", "F13", "heldout", "small", "g5-loc-highsmith", sorted(hs, key=lambda e: e["key"])[:2],
                 ...
                 {"tiles": {"count": 1, "size": [768, 512], "reduce": [3]}, "formats": CODEC_FORMATS, "big_formats_max_inputs": 1,
                  "require_tools": TOOLS_CODECS, "max_bytes": 15 * MiB},
                 tags=["image-codecs", "pipeline:image-codecs-matrix"]))
    # Round-1 fix G21 (F13 distinct-pipeline media, held-out): same x265/libheif phone-style matrix
    # as the validation item above, over same-split Highsmith TIFFs.
    A(lambda: build_item("f13-heldout-phone-matrix-small", "F13", "heldout", "small", "g5-loc-highsmith",
                 sorted(hs, key=lambda e: e["key"])[:2], LIC["highsmith"],
                 "Phone-style HEIC stills and a short synthetic HEVC pan/zoom clip (x265/libheif matrix, as the validation "
                 "phone-matrix item) over two Carol M. Highsmith Archive TIFFs. Provenance-only description.",
                 "phone_matrix.py", 3401,
                 {"inputs": lossless_names(sorted(hs, key=lambda e: e["key"])[:2]), "long_edge": 4032, "heic_qualities": [35, 55],
                  "heic_thumb_at": 1, "video": {"seconds": 2, "fps": 30, "crf": 30, "keyint": 60, "resolutions": [[1920, 1080], [1080, 1920]]},
                  "require_tools": {"ffmpeg": "ffmpeg version 6.1.1-3ubuntu5", "heif-enc": "1.17.6", "pillow": "Pillow 12.3.0"},
                  "max_bytes": 15 * MiB},
                 tags=["phone-style-matrix", "image-codecs", "video", "generated", "pipeline:phone-x265-heif"],
                 real_or_generated="generated"))
    # Round-1 fix G21 (F13 non-benchmark large held-out item): a real NASA-produced documentary mp4
    # -- a production pipeline (NASA video) that no other F13 item uses (NASA only appears in F12,
    # as still photographs), and not a "public-benchmark" item like the Blender films above.
    A(lambda: download_item("f13-heldout-nasa-video-naca-large", "F13", "heldout", "large", "g5-nasa-video-naca",
                    entries_for(sel, "nasa-video-naca", "f13-heldout-nasa-video-naca-large"), LIC["nasa-video"],
                    "A real NASA Image and Video Library documentary mp4 (NASA Ames Research Center, 'Launching the Future of Flight, "
                    "The National Advisory Committee for Aeronautics'). Provenance-only description.",
                    tags=["video", "pipeline:nasa-video"]))
    if skipped:
